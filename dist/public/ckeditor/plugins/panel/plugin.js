...
(function () {
  ...
  */
	CKEDITOR.ui.panel = function (document, definition) {
		...
		if (definition) CKEDITOR.tools.extend(this, definition);
   ...
   */
		render: function render(editor, output) {
					...
					if (this.isFramed) {
						...
						doc.on('keydown', function (evt) {
							...
							if (keystroke == 27 || keystroke == (dir == 'rtl' ? 39 : 37)) {
								if (this.onEscape && this.onEscape(keystroke) === false) evt.data.preventDefault();
							}
						}, this);
			...
			if (this.isFramed) {
				...
				var src = CKEDITOR.env.air ? 'javascript:void(0)' : // jshint ignore:line
   ...
   */
    ...
    */
			markItem: function markItem(index) {
				...
				if (CKEDITOR.env.webkit) item.getDocument().getWindow().focus();
				item.focus();
    ...
    */
			markFirstDisplayed: function markFirstDisplayed(beforeMark) {
				var notDisplayed = function notDisplayed(element) {
				...
				if (CKEDITOR.env.webkit) focused.getDocument().getWindow().focus();
				focused.focus();
    ...
    */
		...
		proto: {
			...
			hide: function hide() {
				if (!this.onHide || this.onHide.call(this) !== true) this.element.setStyle('display', 'none');
			},
