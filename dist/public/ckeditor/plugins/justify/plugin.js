...
(function () {
	function getAlignment(element, useComputedState) {
		...
		var align;
		if (useComputedState) align = element.getComputedStyle('text-align');else {
	...
	function onDirChanged(e) {
		...
		while (node = walker.next()) {
			if (node.type == CKEDITOR.NODE_ELEMENT) {
				...
				if (align == 'left') node.setStyle(style, 'right');else if (align == 'right') node.setStyle(style, 'left');
			}
		}
	}
			...
			for (var i = ranges.length - 1; i >= 0; i--) {
				...
				while (block = iterator.getNextParagraph(enterMode == CKEDITOR.ENTER_P ? 'p' : 'div')) {
					...
					if (cssClassName) {
						...
						if (apply) block.addClass(cssClassName);else if (!className) block.removeAttribute('class');
					} else if (apply) {
