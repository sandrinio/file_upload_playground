	...
	init: function init(editor) {
		...
		function addButton(name, type, title, order, options) {
			...
			editor.ui.add(name, CKEDITOR.UI_PANELBUTTON, {
				...
				onOpen: function onOpen() {
					...
					if (!automaticColor || automaticColor == 'transparent') automaticColor = '#ffffff';
		...
		function renderColors(panel, type, colorBoxId) {
			...
			if (config.colorButton_enableAutomatic !== false) {
				...
				output.push('<a class="cke_colorauto" _cke_focus=1 hidefocus=true' + ' title="', lang.auto, '"' + ' onclick="CKEDITOR.tools.callFunction(', clickFn, ',null,\'', type, '\');return false;"' + ' href="javascript:void(\'', lang.auto, '\')"' + ' role="option" aria-posinset="1" aria-setsize="', total, '">' + '<table role="presentation" cellspacing=0 cellpadding=0 width="100%">' + '<tr>' + '<td colspan="' + colorsPerRow + '" align="center"><span class="cke_colorbox" id="', colorBoxId, '"></span>', lang.auto, '</td>' + '</tr>' + '</table>' + '</a>');
			}
			output.push('<table role="presentation" cellspacing=0 cellpadding=0 width="100%">');

			// Render the color boxes.
			for (var i = 0; i < colors.length; i++) {
				if (i % colorsPerRow === 0) output.push('</tr><tr>');
