	...
	CKEDITOR.plugins.add('autogrow', {
		init: function init(editor) {
			...
			editor.on('instanceReady', function () {
				...
				if (editor.editable().isInline()) editor.ui.space('contents').setStyle('height', 'auto');
	...
	function initIframeAutogrow(editor) {
			...
			editor.on(eventName, function (evt) {
				...
				if (evt.editor.mode == 'wysiwyg') {
					setTimeout(function () {
						...
						if (!maxHeightIsUnlimited) resizeEditor();
					}, 100);
				}
			});
		}
		...
		editor.on('afterCommandExec', function (evt) {
			if (evt.data.name == 'maximize' && evt.editor.mode == 'wysiwyg') {
				if (evt.data.command.state == CKEDITOR.TRISTATE_ON) scrollable.removeStyle('overflow-y');else resizeEditor();
			}
		});
		...
		function resizeEditor() {
			...
			if (maxHeightIsUnlimited) scrollable.setStyle('overflow-y', 'hidden');
			...
			if (!maxHeightIsUnlimited) {
				if (newHeight < configMaxHeight && scrollable.$.scrollHeight > scrollable.$.clientHeight) scrollable.setStyle('overflow-y', 'hidden');else scrollable.removeStyle('overflow-y');
			}
		}
	}
})();
