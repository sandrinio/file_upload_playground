...
(function () {
  ...
  */
   ...
   */
    ...
    * @todo
    */
			addListBlock: function addListBlock(name, multiSelect) {
    ...
    */
			showBlock: function showBlock(name, offsetParent, corner, offsetX, offsetY, callback) {
				...
				if (rtl && (corner == 1 || corner == 4)) left += offsetParent.$.offsetWidth;else if (!rtl && (corner == 2 || corner == 3)) left += offsetParent.$.offsetWidth - 1;
				...
				if (!this._.blurSet) {
					...
					focused.on('blur', function (ev) {
						...
						if (this.visible && !this._.activeChild) {
							...
							if (CKEDITOR.env.iOS) {
								if (!this._.hideTimeout) this._.hideTimeout = CKEDITOR.tools.setTimeout(doHide, 0, this);
							} else {
								doHide.call(this);
							}
						}
				...
				CKEDITOR.tools.setTimeout(function () {
					var panelLoad = CKEDITOR.tools.bind(function () {
						...
						if (CKEDITOR.env.ie) {
							...
							if (scrollParent.getComputedStyle('direction') == 'rtl') {
								...
								if (CKEDITOR.env.ie8Compat) left -= element.getDocument().getDocumentElement().$.scrollLeft * 2;else left -= offsetParent.$.scrollWidth - offsetParent.$.clientWidth;
							}
						}
					...
					CKEDITOR.tools.setTimeout(function () {
						...
						if (CKEDITOR.env.webkit) CKEDITOR.document.getBody().$.scrollTop = scrollTop;
    ...
    * @todo
    */
			hide: function hide(returnFocus) {
				if (this.visible && (!this.onHide || this.onHide.call(this) !== true)) {
					...
					if (focusReturn) {
						...
						if (CKEDITOR.env.webkit && focusReturn.type) focusReturn.getWindow().$.focus();
    ...
    * @todo
    */
			allowBlur: function allowBlur(allow) {
    ...
    * @todo
    */
			showAsChild: function showAsChild(panel, blockName, offsetParent, corner, offsetX, offsetY) {
				// Skip reshowing of child which is already visible.
				if (this._.activeChild == panel && panel._.panel._.offsetParentId == offsetParent.getId()) return;
    ...
    * @todo
    */
			hideChild: function hideChild(restoreFocus) {
